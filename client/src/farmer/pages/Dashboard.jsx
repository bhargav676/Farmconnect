...
const Dashboard = () => {
  ...
  const revenueChartData = { labels: purchases.map((p) => new Date(p.createdAt).toLocaleDateString()), datasets: [{ label: 'Revenue (₹)', data: purchases.map((p) => p.totalPrice), borderColor: '#16a34a', backgroundColor: 'rgba(22, 163, 74, 0.1)', fill: true, tension: 0.3 }] };
